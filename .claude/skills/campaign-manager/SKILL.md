...
1. **Extract before extend.** Phase 1 asks good questions first, references the Operator Playbook (`operator/the operator/` when populated), then synthesises a Brief. Briefs attribute "[the operator's read]" vs "[AI extension]" vs "[AI synthesis]".
...
4. **Decision-first, reference-collapsed.** Every operator surface puts the next action on top; history/audit/rationale go below the fold in `<details>`. Cold-start safe.
...
1. Author the Plan per [`plan.md`](../../../docs/specs/plan.md). The asset-list table MUST carry: `# | Asset | Review shape | Form | Ships | Copy file | Owner agent | Phase | Target date | Depends on | Notes`.
...
For each Plan asset, in sequence (or parallel where dependencies allow):
